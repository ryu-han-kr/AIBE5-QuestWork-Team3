...
import { User, Briefcase, DollarSign, Award, Settings, Save, X, Link2, Lock, Calendar as CalendarIcon } from 'lucide-react'
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  {isEditing ? (
                      ...
                      <div className="space-y-2 flex-1 max-w-md">
                        ...
                        <Label>소개글</Label>
                        <Textarea name="intro" rows={3} value={draft?.intro} onChange={handleInputChange} placeholder="전문 분야나 프로젝트 경험을 자유롭게 적어주세요." className="bg-background leading-relaxed" />
                      </div>
                  ) : (
                      <div>
                        <h1 className="text-7xl font-black tracking-tighter text-foreground">{profile.nickname}</h1>
        ...
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-12">
                    ...
                    <Card className="rounded-3xl p-6 bg-card">
                      ...
                      <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto">
                        {TECH_STACK_OPTIONS.map((tech) => (
                          <div key={tech} className="flex items-center gap-2 cursor-pointer">
                            <input
                              type="checkbox"
              ...
              {/* 찜한/진행 중 퀘스트 */}
              <section>
                <h2 className="mb-6 text-2xl font-black flex items-center gap-2"><Award size={24} className="text-primary"/> 내 퀘스트 현황</h2>
                  ...
                  <div className="lg:col-span-1">
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {MOCK_QUESTS.filter(quest => questFilter === '전체' || quest.status === questFilter).map(quest => {
                        ...
                        return (
                          <Card key={quest.id} className="rounded-2xl p-4 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between">
                              ...
                              <Badge variant={quest.status === '진행중' ? 'default' : 'secondary'} className="ml-4">
                                {quest.status}
                              </Badge>
                            </div>
                          </Card>
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                  <DialogContent>
                    ...
                    <DialogFooter>
                      ...
                      <Button onClick={addQuest} disabled={!newQuestTitle.trim()}>추가</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </section>
