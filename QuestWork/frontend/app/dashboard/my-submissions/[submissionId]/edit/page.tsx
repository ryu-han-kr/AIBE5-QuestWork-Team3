  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background">
        <GlobalNav />
        <DashboardShell>
          <div className="py-16 text-center text-foreground-muted">불러오는 중...</div>
        </DashboardShell>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-background">
        ...
        <DashboardShell>
          <div className="py-16 text-center">
            ...
            <p className="mt-2 text-foreground-muted">제출물이 성공적으로 수정되었습니다.</p>
            {original && (
              <Card className="mx-auto mt-6 max-w-sm border border-border p-4 text-left text-sm">
                <p className="font-semibold text-foreground">{original.submissionTitle}</p>
                <div className="mt-2 space-y-1 text-foreground-muted">
                  <p>버전: v{original.versionNo}</p>
                  <p>상태: {STATUS_LABEL[original.status] ?? original.status}</p>
                  <p>수정일: {new Date(original.updatedAt).toLocaleString('ko-KR')}</p>
                </div>
              </Card>
            )}
      ...
      <DashboardShell>
        <div className="mb-6">
          <nav className="mb-4 flex items-center gap-2 text-sm text-foreground-muted">
            <Link href="/dashboard/my-submissions" className="hover:text-foreground">
              내 제출물
            </Link>
            <span>/</span>
          ...
          <h1 className="mt-1 text-3xl font-bold text-foreground">제출물 수정</h1>
          {original && (
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          {/* 제출 링크 */}
          <Card className="border border-border p-6">
            <h2 className="mb-4 text-base font-semibold text-foreground">제출 링크 수정</h2>
